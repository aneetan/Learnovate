...
import AdditionalInfoMentorStep from "../components/Auth/AdditionalInfoMentorStep"
import MenteeProfileStep from "../components/Auth/MenteeProfileStep"
import ProfessionalInfoMentorStep from "../components/Auth/ProfessionalInfoMentorStep"
import DocumentUploadsMentorStep from "../components/Auth/DocumentUploadsMentorStep"
      ...
      <div className={`bg-white rounded-lg shadow-md p-10 w-full max-w-[850px] border border-gray-200 transition-all duration-300 relative z-1 flex flex-col justify-center ${step > 2 ? 'max-w-[950px]' : ''}`}>
        <div className="flex items-center mb-8">
          {step > 1 && (
            <button
              type="button"
              className="text-gray-700 hover:text-[#26A69A] transition-colors duration-200 mr-4"
              onClick={handleBack}
              disabled={loading}
            >
              <svg
          ...
          <h2 className="text-center flex-grow text-gray-900 font-bold text-3xl relative after:content-[''] after:absolute after:bottom-[-0.5rem] after:left-1/2 after:transform after:-translate-x-1/2 after:w-12 after:h-1 after:bg-[#26A69A] after:rounded">Register for Learnovate</h2>
        </div>
        ...
        <form onSubmit={step === (formData.role === "mentor" ? 5 : 4) ? handleSubmit : handleNext}>
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <button
              type="submit"
              className="flex bg-[#26A69A] text-white px-6 py-3 rounded-lg font-medium cursor-pointer hover:bg-[#20897f] hover:-translate-y-px transition-all duration-200 disabled:bg-gray-300 disabled:cursor-not-allowed"
            ...
            {step === 3 && formData.role === "mentee" && (
              <button
                type="button"
                className="flex bg-gray-500 text-white px-6 py-3 rounded-lg font-medium cursor-pointer hover:bg-gray-600 hover:-translate-y-px transition-all duration-200 disabled:bg-gray-300 disabled:cursor-not-allowed"
                onClick={handleSkip}
                disabled={loading}
              >
                Skip
